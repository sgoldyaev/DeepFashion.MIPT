...
from ADGAN.data.base_dataset import BaseDataset, get_transform
from ADGAN.data.image_folder import make_dataset
...
class KeyDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        pattern = 'fashion|WOMEN|MEN|Denim|Jackets_Vests|Pants|Shirts_Polos|Shorts|Suiting|Sweaters|Sweatshirts_Hoodies|Tees_Tanks|Blouses_Shirts|Cardigans|Denim|Dresses|Graphic_Tees|Jackets_Coats|Leggings|Pants|Rompers_Jumpsuits|Shorts|Skirts|Sweaters|Sweatshirts_Hoodies|Tees_Tanks|\w{11}|\w+'
        # segmentation
        # SP1_name = self.split_name(P1_name, 'semantic_merge3')
        # SP1_path = os.path.join(self.dir_SP, SP1_name)
        # SP1_path = SP1_path[:-4] + '.npy'
        # SP1_path = os.path.join('./deepfashion/fashion_resize/testK/', P1_name + '.npy')
        # SP1_data = np.load(SP1_path)
        SP1_name = re.findall(pattern, P1_name)
        SP1_path = os.path.join('./deepfashion/semantic_merge3/', SP1_name[1], SP1_name[2], SP1_name[3], SP1_name[4] + '.npy')
        SP1_data = np.load(SP1_path)
